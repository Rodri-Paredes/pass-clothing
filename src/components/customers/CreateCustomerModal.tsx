...
export default function CreateCustomerModal({ open, onClose, onCreated }: Props) {
  ...
  return <Modal isOpen={open} onClose={onClose} title="Nuevo cliente"><form noValidate onSubmit={submit} className="space-y-4"><Input label="Nombre *" value={firstName} onChange={e => setFirstName(e.target.value)} autoFocus/><Input label="Teléfono *" value={phone} onChange={e => setPhone(e.target.value)} inputMode="tel"/><div className="grid gap-3 sm:grid-cols-2"><Input label="Apellido" value={lastName} onChange={e => setLastName(e.target.value)}/><Input label="Email" type="email" value={email} onChange={e => setEmail(e.target.value)}/></div><Input label="Instagram" value={instagram} onChange={e => setInstagram(e.target.value)}/>{existing && <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm"><p>Ya existe un cliente con este número.</p><p className="mt-1 font-medium">{existing.full_name || existing.first_name} · {existing.customer_code} · {existing.phone}</p><Button type="button" className="mt-2" size="sm" onClick={() => { onCreated(existing); onClose(); }}>Seleccionar cliente existente</Button></div>}{error && <p className="text-sm text-red-600">{error}</p>}<div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={onClose}>Cancelar</Button><Button type="submit" disabled={saving}>{saving ? 'Guardando…' : 'Crear y seleccionar'}</Button></div></form></Modal>;
}
