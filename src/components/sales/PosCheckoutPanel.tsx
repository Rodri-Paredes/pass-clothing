...
export default function PosCheckoutPanel(props: Props) {
  ...
  const panel = <div className="flex h-full flex-col bg-white">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto px-5 py-4">
      <section className="pb-4"><p className="mb-2 text-xs font-semibold uppercase tracking-wider text-surface-500">Cliente</p><CustomerSelector value={props.customer} onChange={props.onCustomerChange} onCreate={props.onCreateCustomer}/>{props.customer && props.crewContext?.loyalty_enabled && <div className="mt-2 flex items-center justify-between bg-violet-50 px-3 py-2 text-xs text-violet-900"><strong>Puntos</strong><span className="font-bold">{props.crewContext.points_balance || 0}</span></div>}{props.crewContext?.active && <div className="mt-2 bg-amber-50 px-3 py-2 text-xs text-amber-900"><div className="flex items-center justify-between"><strong>PASS Crew · {props.crewContext.plan_name}</strong><span>Vence {new Date(props.crewContext.expires_at || '').toLocaleDateString('es-BO')}</span></div>{props.crewContext.benefits.length > 0 && <p className="mt-1 text-amber-800">{props.crewContext.benefits.map(item => item.name).join(' · ')}</p>}</div>}</section>
