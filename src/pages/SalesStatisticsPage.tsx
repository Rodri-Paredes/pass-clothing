...
export default function SalesStatisticsPage() {
  ...
  const load = useCallback(async () => {
    setError('');
    try {
      const rows = await Promise.all(dimensions.map(({ key }) => salesStatisticsService.unitsBreakdown(key, instant(range.start), instant(range.end), branchId || undefined, topN)));
