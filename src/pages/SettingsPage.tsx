...
const SettingsPage: React.FC = () => {
  ...
  return <div className="mx-auto max-w-3xl p-8">
    ...
    {loyalty && <Card className="mb-8"><div className="space-y-5 p-6"><div><h2 className="text-xl font-semibold">Fidelidad / Puntos</h2><p className="mt-1 text-sm text-gray-500">Configura la regla sin hardcodear valores comerciales. Los cambios aplican a nuevas ventas.</p></div><label className="flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={loyalty.enabled} onChange={e => setLoyalty({...loyalty, enabled: e.target.checked})}/> Activar acumulación de puntos</label><div className="grid gap-4 sm:grid-cols-2"><label className="text-sm">Puntos otorgados<input type="number" min="0" step="0.01" value={loyalty.points_per_currency_unit} onChange={e => setLoyalty({...loyalty, points_per_currency_unit: Number(e.target.value)})} className="mt-1 w-full rounded border px-3 py-2"/></label><label className="text-sm">Por cada Bs<input type="number" min="0.01" step="0.01" value={loyalty.currency_unit_amount} onChange={e => setLoyalty({...loyalty, currency_unit_amount: Number(e.target.value)})} className="mt-1 w-full rounded border px-3 py-2"/></label><label className="text-sm">Compra mínima<input type="number" min="0" step="0.01" value={loyalty.minimum_purchase_amount} onChange={e => setLoyalty({...loyalty, minimum_purchase_amount: Number(e.target.value)})} className="mt-1 w-full rounded border px-3 py-2"/></label><label className="text-sm">Máximo por venta<input type="number" min="0" placeholder="Sin límite" value={loyalty.max_points_per_sale ?? ''} onChange={e => setLoyalty({...loyalty, max_points_per_sale: e.target.value ? Number(e.target.value) : null})} className="mt-1 w-full rounded border px-3 py-2"/></label><label className="text-sm">Redondeo<select value={loyalty.rounding_strategy || 'floor'} onChange={e => setLoyalty({...loyalty, rounding_strategy: e.target.value as 'floor' | 'round' | 'ceil'})} className="mt-1 w-full rounded border px-3 py-2"><option value="floor">Hacia abajo</option><option value="round">Normal</option><option value="ceil">Hacia arriba</option></select></label></div><p className="text-xs text-gray-500">La expiración permanece desactivada en esta fase. El canje sigue preparado pero inactivo.</p><div className="border-t pt-4"><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={loyalty.redemption_enabled} onChange={e => setLoyalty({...loyalty, redemption_enabled: e.target.checked})}/> Habilitar canje</label><p className="mt-1 text-xs text-gray-500">No se implementa equivalencia de puntos a bolivianos hasta definirla.</p></div><div className="flex items-center justify-between border-t pt-4"><span className="text-sm text-gray-500">{message}</span><Button onClick={saveLoyalty} disabled={saving}>{saving ? 'Guardando…' : 'Guardar puntos'}</Button></div></div></Card>}
